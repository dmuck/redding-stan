...
It's possible to run ReddingStan within an R environment with the `processx` [library](LINK.ME). To start a new background process, the executable should already be built and ready to run. 

Using the Bernoulli model as an example, it would look like this:
...
ReddingStan facilitates algorithmic development by smuggling log probability and gradient information out of a Stan program. 

To start with a simple example, the Bernoulli model in Stan will help illustrate the utility of ReddingStan. 

Though a bit contrived, this example will demonstrate how to conduct gradient descent on a single-parameter model. The Bernoulli model in Stan is written as
...
### Gradient Descent using ReddingStan

First, load the `processx` library, source in some helper functions, and initialize the model with the data: 
...
Now that the model is ready to go, it's time to set some initial values for the algorithm. First, extract the number of (unconstrained) parameters from the Stan model 
...
and use that to generate random parameter value(s) for the `N` number of parameters, which in this case will be one: 
...
Now it's time to conduct gradient descent using ReddingStan. ReddingStan provides the gradient information necessary for this algorithm, and it can be extracted using the helper function `get_gradient()`. The following code illustrates how:
